                ...
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;"
                       ...
                       @click.prevent="editCart('check',item)">
...
export default {
  ...
  computed: {
    ...
    cartmodal () {
      return this.$store.state.mdShowCart
    },
    checkoutCount () {
      var checkoutCount = 0;
      this.cartList.forEach(item => {
        if (item.checked == 1) {
          checkoutCount += item.productNum
        }
      });
  ...
  methods: {
    getCartList () {
      axios.get("/users/cartlist").then((response) => {
        ...
        this.checkAllFlag = this.cartList.every(item => {
    ...
    checkAll () {
      ...
      this.cartList.forEach(item => {
        item.checked = this.checkAllFlag == true ? "1" : "0"
      });
      axios.post("/users/checkall", {
        checked: this.checkAllFlag == true ? "1" : "0"
      }).then(response => {
        var res = response.data
      })
    },
