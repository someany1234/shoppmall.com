...
export default {
  ...
  methods: {
    init () {
      axios.post("/users/orderconfirm").then(response => {
        this.orderconfirm = response.data.result
        this.orderconfirm.forEach((item) => {
          if (item.checked == "1") {
            this.subtotal += item.salePrice * item.productNum
          }
        });
        this.shipping = this.subtotal > 100 ? 0 : 12
        this.tax = (this.subtotal * 17) / 100
